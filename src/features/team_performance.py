...
def calc_team_finish_features(results_df: pd.DataFrame) -> pd.DataFrame:
    ...
    results_df['TeamDNFRateLast5'] = results_df.groupby(['TeamName', 'Year'])['Finished'].transform(lambda x: (~x).shift(1).rolling(5).mean())
...
def calc_team_pit_features(laps_df: pd.DataFrame) -> pd.DataFrame:
    ...
    pit_laps_df = pit_laps_df.groupby(['Team', 'Year', 'RoundNumber'])['PitTime'].agg('mean').reset_index()
    ...
    pit_laps_df['AvgTeamPitTimeLast3'] = pit_laps_df.groupby(['Team', 'Year'])['AvgTeamPitTime'].transform(lambda x: x.shift(1).rolling(3).mean())
    
    # Calculate average pit time by team for previous 5 races
    pit_laps_df['AvgTeamPitTimeLast5'] = pit_laps_df.groupby(['Team', 'Year'])['AvgTeamPitTime'].transform(lambda x: x.shift(1).rolling(5).mean())
